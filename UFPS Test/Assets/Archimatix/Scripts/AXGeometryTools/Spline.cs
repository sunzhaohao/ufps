...
namespace AXGeometryTools
{
	...
	public class Spline 
	{
		...
		public Spline(Path path, bool _isClosed, float breakP, float breakN)
		{
			isClosed 			= _isClosed;
		...
		public Spline(List<Vector2> verts,  bool _isClosed, float breakP, float breakN)
		{
			controlVertices 	= verts;
			breakAngleGeom = breakP;
			breakAngleNormals 	= breakN;
		...
		{ 	
			...
			if (path != null && path.Count > 0)
			{
				...
				foreach (IntPoint ip in path)
					controlVertices.Add (AXGeometryTools.Utilities.IntPt2Vec2(ip));
		...
		public void initializeCurveMetrics(Boolean allowClosingPoint = false) {
			...
			float perpAngleFirst = 0;
			float perpAngleLast = 0;
			...
			for (int i=1; i<=controlVertices.Count; i++) {
				...
				Matrix4x4 m  = Matrix4x4.TRS(new Vector3 (p.x, 0, p.y), Quaternion.Euler(-90, 0, tmp_ang), new Vector3(scalerVal, 1, 1));
				//Debug.Log("m["+i+"]="+m);
			...
			if (shapeState == ShapeState.Open)
			{
				...
				{
					if ( Mathf.Abs(bevelAngles[0]) < minAng  && Mathf.Abs(bevelAngles[1]) < minAng  && edgeLengths[0] <= edgeLengths[1]*1.1f)
					{
						isBeveledA = true;
						rotA += bevelAngles[1]; 
		...
		public Matrix4x4 matrixAtLength(float len) 
		{
			...
			for (int i=0; i<controlVertices.Count-1; i++)
			{
				...
				if (tryer > len)
				{
					float remainder = len - cummulativeDistance;
					float percentage = remainder/v2.magnitude;

					Vector2 pos2D = Vector2.Lerp(p, np,  percentage);

					Vector3 pos3D = new Vector3(pos2D.x, 0, pos2D.y);

					float rot = edgeRotations[prev_i];
					//Debug.Log(">>> remainder="+remainder);

					return Matrix4x4.TRS(pos3D, Quaternion.Euler(0, rot, 0), Vector3.one);
				}
...
//
		...
		// At the end call creatInsetSplines (spanSplines),
		...
		public void groupNearBreakAngleVertices(float min_len)
		{
...
//			}
		...
		public void getInsetCornerSplines(float inset)
		{
			...
			// Now combine subSplines where the endpoints are colser than the 
			...
			//Debug.Log("** ** ** ** ----->>>> " + groupedCornerNodes.Count);
			...
			Vector2 leftInsertedVert_0 = Vector2.zero;


			int terminIndex = (shapeState == ShapeState.Open) ? 0 : 0;

			//Debug.Log("groupedCornerNodes.Count="+groupedCornerNodes.Count+", terminIndex="+terminIndex);
			for ( int i=0; i<groupedCornerNodes.Count-terminIndex; i++)
			{
				//Debug.Log("Group "+i+ " ========================================================================>>>>>>");
				...
				if (i > 0 || shapeState == ShapeState.Closed)
				{
					...
					for (int con_i=0; con_i<controlVertices.Count; con_i++)
					{
						...
						//Debug.Log(con_i + ": this_index="+this_index+", prev_index="+prev_index +" ::: len = " + segment_lengths[controlVerts_cursor] + ", remainingRunningDist="+remainingRunningDist);

						if (segment_lengths[controlVerts_cursor] > remainingRunningDist)
						{
							// OK - found the last regular control vert in the inset before this group.
							...
							Vector2 begVert = Vector2.Lerp (controlVertices[prev_index], controlVertices[this_index], percentage);


							
							cornerGroupSpline.controlVertices.Add(begVert);
							...
							//Debug.Log("["+i+"] prevGroup_i="+prevGroup_i +" of "+ (groupedCornerNodes.Count-1) + " :::: " + groupedCornerNodes[prevGroup_i][groupedCornerNodes[prevGroup_i].Count-1]);

							if (i == 0)
								leftInsertedVert_0 = begVert;
							...
							else 
							{
								...
								insetSpanSplines[prevGroup_i].controlVertices.Add(begVert);
				...
				// RIGHTSIDE INSET: now add END lerp...
				...
				if (  i < (groupedCornerNodes.Count-1)  || shapeState == ShapeState.Closed)
				{
					...
					if (i == groupedCornerNodes.Count-1 && shapeState == ShapeState.Closed)
					{
						//Debug.Log("Adding END POINT" + leftInsertedVert_0);
						insetSpanSplines[i].controlVertices.Add(leftInsertedVert_0);
		...
		public static void testSpline()
		{
			...
			pathh.Add(new IntPoint(3000, AXGeometryTools.Utilities.IntPointPrecision));
			pathh.Add(new IntPoint( 0,   AXGeometryTools.Utilities.IntPointPrecision));
			//pathh.Add(new IntPoint(0, 0));
