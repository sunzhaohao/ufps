...
//				{
					...
					Type typeBasedOnString = po.generator.generatorHandlerType;
...
//			}
		...
		public virtual int customNodeGUIZone_4(int cur_y, AXNodeGraphEditorWindow editor, AXParametricObject po)
		{
		...
		public virtual void drawControlHandles(ref List<string> visited, Matrix4x4 consumerM, bool beingDrawnFromConsumer)
		{
			...
			if (parametricObject.handles != null)
			{
				...
				foreach(AXHandle han in parametricObject.handles)
				{
					...
					float tangent = .2f;
					try {	
						tangent = (float) parametricObject.parseMath(han.tangent);
					} catch(Exception e) {
						parametricObject.codeWarning = "1. Handle error: Please check syntax of: \"" + han.tangent+"\" " + e ;
					}
					...
					float radius = .2f;
					try {	
						radius = (float) parametricObject.parseMath(han.radius);
					} catch(Exception e) {
						parametricObject.codeWarning = "2. Handle error: Please check syntax of: \"" + han.radius+"\" "+ e ;
					}
...
//
					...
					case  AXHandle.HandleType.Point:
						...
						if (ArchimatixEngine.snappingOn())
							pos = AXGeometryTools.Utilities.SnapToGrid(pos, parametricObject.model.snapSizeGrid);
					...
					case AXHandle.HandleType.Angle:
						...
						float ang = 10;
						try {	
							ang = (float)parametricObject.parseMath(han.angle );
						} catch(Exception e) {
							parametricObject.codeWarning = "Handle error: Please check syntax of: \"" + han.angle+"\" "+e.Message;
						}
						try {	
							len = (float) parametricObject.parseMath(han.len);
						} catch(Exception e) {
		...
		public void drawPaths(AXParameter p)
		{
			...
			if (actualPaths != null)
			{
				...
				{
					Vector3[] verts3d = AXGeometryTools.Utilities.path2Vec3s(path, Axis.Z, (p.drawClosed  || (p.shapeState == ShapeState.Closed)));
	...
	public class GeneratorHandler2D : GeneratorHandler {
		...
		{ 
			...
			if (ArchimatixEngine.sceneViewState != ArchimatixEngine.SceneViewState.AddPoint)
			{
				...
				if (EditorGUI.EndChangeCheck())
				{
					...
					if (ArchimatixEngine.snappingOn())
						cen = AXGeometryTools.Utilities.SnapToGrid(cen, parametricObject.model.snapSizeGrid);
			...
			{	
				...
				if (p.transformedButUnscaledOutputPaths != null)
				{
					for (int i=0; i<p.transformedButUnscaledOutputPaths.Count; i++)
					{
						...
						verts3d = AXGeometryTools.Utilities.path2Vec3s(p.transformedButUnscaledOutputPaths[i], Axis.Z, (p.shapeState == ShapeState.Closed));
				...
				if (p.offset != 0 && p.transformedAndScaledButNotOffsetdOutputPaths != null)
				{
					...
					{
						verts3d = AXGeometryTools.Utilities.path2Vec3s(p.transformedAndScaledButNotOffsetdOutputPaths[i], Axis.Z, (p.shapeState == ShapeState.Closed));
				...
				*/
				...
				foreach(Path path in p.getPaths())
				{
					...
					verts3d = AXGeometryTools.Utilities.path2Vec3s(path, Axis.Z, (p.drawClosed  || (p.shapeState == ShapeState.Closed)));
		...
		public  void drawHandlesUnselectedDiabled()
		{
			...
			if (! p.drawClosed)
			{
				...
				foreach(Path path in p.getPaths())//p.getTransformedControlPaths())
				{
					...
					verts3d = AXGeometryTools.Utilities.path2Vec3s(path, Axis.Z, (p.shapeState == ShapeState.Closed));
	...
	public class GeneratorHandler3D : GeneratorHandler {
		...
		public override void drawTransformHandles(List<string> visited, Matrix4x4 consumerM, bool addLocalMatrix = false)
		{
			...
			Matrix4x4 context 		= parametricObject.model.transform.localToWorldMatrix * generator.parametricObject.worldDisplayMatrix * (parametricObject.getAxisRotationMatrix() * parametricObject.getLocalAlignMatrix()).inverse * generator.localMatrix.inverse;
			...
			case UnityEditor.Tool.Move:
				...
				if(EditorGUI.EndChangeCheck())
				{
					...
					// convert to local space
					//cen = (consumerM*parametricObject.getAxisRotationMatrix() * parametricObject.getLocalAlignMatrix()).inverse.MultiplyPoint3x4(cen);
					...
					generator.parametricObject.worldDisplayMatrix = context  * Matrix4x4.TRS(cen, rot, Vector3.one)* parametricObject.getAxisRotationMatrix() * parametricObject.getLocalAlignMatrix();
					generator.adjustWorldMatrices();
				}
			...
			case Tool.Rotate:
				...
				if(EditorGUI.EndChangeCheck())
				{
					...
					if (ArchimatixEngine.snappingOn())
						rotAngs = AXGeometryTools.Utilities.SnapToGrid(rotAngs, 10);
					...
					generator.parametricObject.worldDisplayMatrix = context  * Matrix4x4.TRS(cen, rot2, Vector3.one)* parametricObject.getAxisRotationMatrix() * parametricObject.getLocalAlignMatrix();
					generator.adjustWorldMatrices();
		...
		public override void drawControlHandles(ref List<string> visited, Matrix4x4 consumerM, bool beingDrawnFromConsumer)
		{
			...
			*/
			...
			if (beingDrawnFromConsumer)
			{
				...
				consumerM *= parametricObject.getAxisRotationMatrix() * parametricObject.getLocalAlignMatrix();
			} 
			else
			{
		...
		public override void drawBoundsHandles(Matrix4x4 consumerM, bool forceDraw=false)
		{
			...
			Handles.matrix = parametricObject.model.transform.localToWorldMatrix * generator.parametricObject.worldDisplayMatrix;//  consumerM * parametricObject.getAxisRotationMatrix() * parametricObject.getLocalAlignMatrix() ;
			...
			if (doX)
			{
				...
				if(EditorGUI.EndChangeCheck())
				{
					...
					if (ArchimatixEngine.snappingOn())
						posx.x = AXGeometryTools.Utilities.SnapToGrid(posx.x, parametricObject.model.snapSizeGrid)+lever;


					P_X.initiateRipple_setFloatValueFromGUIChange(2*(posx.x-lever));
			...
			if (doY)
			{
				...
				if(EditorGUI.EndChangeCheck())
				{
					...
					if (ArchimatixEngine.snappingOn())
						posy.y = AXGeometryTools.Utilities.SnapToGrid(posy.y, parametricObject.model.snapSizeGrid)+lever;

					//Debug.Log(posy.y + " ... " + lever);
					P_Y.initiateRipple_setFloatValueFromGUIChange((posy.y-lever));
			...
			if (doZ)
			{
				...
				{					
					...
					if (ArchimatixEngine.snappingOn())
						posz.z = AXGeometryTools.Utilities.SnapToGrid(posz.z, parametricObject.model.snapSizeGrid)+lever;

					P_Z.initiateRipple_setFloatValueFromGUIChange(2*(posz.z-lever));
		...
		public override void drawTextureHandles(Matrix4x4 consumerM)
		{
			...
			//Handles.matrix = consumerM * parametricObject.getLocalAlignMatrix() * parametricObject.getAxisRotationMatrix();
