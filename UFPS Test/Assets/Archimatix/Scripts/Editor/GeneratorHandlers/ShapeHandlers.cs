...
namespace AX.GeneratorHandlers
{
	...
	{
	/*
		[MenuItem("GameObject/3D Object/Archimatix Nodes/Shapes/Empty Shape")]
		static void InitEmpty() {
			AXEditorUtilities.addNodeToCurrentModel("Shape");
		}
		
		[MenuItem("GameObject/3D Object/Archimatix Nodes/Shapes/ShapeDistributor")]
		static void InitShapeDistributor() {
			AXEditorUtilities.addNodeToCurrentModel("ShapeDistributor");
		}
		
		[MenuItem("GameObject/3D Object/Archimatix Nodes/Shapes/ShapeSegmentor")]
		static void InitShapeSegmentor() {
			AXEditorUtilities.addNodeToCurrentModel("ShapeSegmentor");
		}
		
		[MenuItem("GameObject/3D Object/Archimatix Nodes/Shapes/FreeCurve")]
		static void InitFreeCurve() {
			AXEditorUtilities.addNodeToCurrentModel("FreeCurve");

			AXModelEditorWindow edwin = (AXModelEditorWindow) EditorWindow.GetWindow(typeof(AXModelEditorWindow));
			if (edwin != null)
				edwin.setEditMode(AXModelEditorWindow.EditMode.AddPoint);

		}
		
		[MenuItem("GameObject/3D Object/Archimatix Nodes/Shapes/ShapeMerger")]
		static void InitShapeMerger() {
			AXEditorUtilities.addNodeToCurrentModel("ShapeMerger");
		}
		*/
		...
		*/
		...
		public override void drawControlHandles(ref List<string> visited, Matrix4x4 consumerM, bool beingDrawnFromConsumer)
		{
			...
			if(EditorGUI.EndChangeCheck())
			{
				...
				gener.P_Separation.initiateRipple_setFloatValueFromGUIChange( Mathf.Max(2*pos.x, .1f) );
