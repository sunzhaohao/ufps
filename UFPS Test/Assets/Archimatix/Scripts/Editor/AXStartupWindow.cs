...
public class AXStartupWindow : EditorWindow
{
	...
	void OnGUI()
	{
		...
		GUI.DrawTexture(headerRect, headerPic, ScaleMode.StretchToFill, false);
...
*/
		...
		GUILayout.BeginHorizontal();
		{
			showAcknowledgements = EditorGUILayout.Foldout(showAcknowledgements, "Acknowledgments");
		...
		GUILayout.BeginHorizontal();
		{
			if (GUILayout.Button("<b>Documentation</b>\n<size=12>Complete manual, examples tips</size>", richButtonStyle, GUILayout.MaxWidth(260), GUILayout.Height(36)))
				Application.OpenURL("http://archimatix.com/documentation");
			
			if (GUILayout.Button("<b>Rate it</b>\n<size=12>Leave a review on the Asset Store</size>", richButtonStyle, GUILayout.Height(36)))
			{
		...
		GUILayout.BeginHorizontal();
		{
			...
			// UNITY FORUM
			if (GUILayout.Button("<b>Support Forum</b>\n<size=12>Unity Community</size>", richButtonStyle, GUILayout.MaxWidth(172), GUILayout.Height(36)))
