...
namespace AX.Generators 
{
	 ...
	 */
	public class NoiseDeformer : AX.Generators.Generator3D
	{
		...
		public override void init_parametricObject() 
		{
			...
			parametricObject.addParameter(AXParameter.DataType.Float, 	"Frequency", 2f);
				...
				for (int i = 0; i < P_Input.DependsOn.meshes.Count; i++) {
					...
					else
					{
						...
						for (int j = 0; j < verts.Length; j++) {
							...
							float curveScaleVal 	= modifierCurve.Evaluate( vert.y);

							float pang = pval*6*100 ;
							pang *= modifierCurve.Evaluate( vert.y) * curveScaleVal;
