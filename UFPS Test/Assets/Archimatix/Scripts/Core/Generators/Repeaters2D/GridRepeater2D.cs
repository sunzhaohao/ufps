		...
		public virtual void initializeBays(string pName)
		{
			...
			case "Node Shape":
				if (repeaterToolU != null)
					repeaterToolU.P_Bay.initiateRipple_setFloatValueFromGUIChange( Mathf.Abs(nodeSrc_p.parametricObject.bounds.size.x)*1.5f);
				if (repeaterToolV != null)
					repeaterToolV.P_Bay.initiateRipple_setFloatValueFromGUIChange( Mathf.Abs(nodeSrc_p.parametricObject.bounds.size.y)*1.5f);
				break;
			case "Cell Shape":
				if (repeaterToolU != null)
					repeaterToolU.P_Bay.initiateRipple_setFloatValueFromGUIChange(  Mathf.Abs(cellSrc_p.parametricObject.bounds.size.x)*1.1f);
				if (repeaterToolV != null)
					repeaterToolV.P_Bay.initiateRipple_setFloatValueFromGUIChange(  Mathf.Abs(cellSrc_p.parametricObject.bounds.size.y)*1.1f);
				break;
			}
		}
