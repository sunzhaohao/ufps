...
namespace AX.Generators
{
	...
	public class Mesher : AX.Generators.Generator3D, IMeshGenerator
	{
		...
		{
			GUIColor 		= new Color(.63f,.8f,.63f,.8f);
		...
		public GameObject generateFirstPass(AXParametricObject initiator_po, bool makeGameObjects, AXParameter plan_p, AXParameter sec_p, Matrix4x4 endCapShiftM, bool renderToOutputParameter=true, bool addLocalMatrix = true)
		{
			...
			//int seglen = (int) (plan_p.seglen * AXGeometryTools.Utilities.IntPointPrecision);
			...
			float height = (secPaths != null) ? (Clipper.GetBounds (secPaths).top/AXGeometryTools.Utilities.IntPointPrecision) : 0;
			...
			*/
			...
			{
				//Debug.Log ("Do Plan AXClipperLib.PolyTree: " + parametricObject.Name);


				// BOUNDS FOR ASSEMBLY
				Rect plan_bnds 	=  AXGeometryTools.Utilities.getClipperBounds(Clipper.PolyTreeToPaths(plan_p.polyTree));
				Rect sec_bnds = new Rect();

				 if (sec_p != null)
					sec_bnds= ( sec_p.polyTree != null ) ? AXGeometryTools.Utilities.getClipperBounds(Clipper.PolyTreeToPaths(sec_p.polyTree)) : AXGeometryTools.Utilities.getClipperBounds(sec_p.paths);
				...
				else // COMBINED (MakeGO's or not)
				{
					...
					if (secPaths != null)
					{
						...
						{					
							...
							//seint seglen = getSegLenBasedOnSubdivision(glen = (int) (plan_p.seglen * AXGeometryTools.Utilities.IntPointPrecision);
							...
							if (sec_p != null && (hasSides || hasBackfaces))
							{
								...
								foreach(Path planPath in planPaths)
								{
									...
									if (hasSides)
									{
										...
										//AXGeometryTools.Utilities.calculateMeshTangents(ref sidesMesh);
									...
									if (hasBackfaces)
									{
										...
										sidesMesh = new Mesh();
										try {
											planSweeper.generate(ref sidesMesh, planSpline, sectionSplineRev, texy);
										} catch  {
									...
									{
										Matrix4x4 capBTransform = AXGeometryTools.Utilities.getEndTransformB(planPath);
							...
							if (topCap)
							{
								...
								if (topx != 0)
								{
									...
									co.AddPaths (AXGeometryTools.Utilities.cleanPaths(planPaths), AXClipperLib.JoinType.jtMiter, AXClipperLib.EndType.etClosedPolygon); //JoinType.jtSquare, AXClipperLib.EndType.etClosedPolygon);
									resPolytree = new AXClipperLib.PolyTree();
									co.Execute (ref resPolytree, (double)(topx * AXGeometryTools.Utilities.IntPointPrecision));
								}

								topMesh = AXPolygon.triangulate(resPolytree, topCapTex, seglen);
							...
							if (botCap) 
							{
								...
								if (botx != 0)
								{
									...
									co.AddPaths (AXGeometryTools.Utilities.cleanPaths(planPaths), AXClipperLib.JoinType.jtMiter, AXClipperLib.EndType.etClosedPolygon); //JoinType.jtSquare, AXClipperLib.EndType.etClosedPolygon);
									resPolytree = new AXClipperLib.PolyTree();
									co.Execute (ref resPolytree, (double)(botx * AXGeometryTools.Utilities.IntPointPrecision));
								}
								botMesh = AXPolygon.triangulate(resPolytree, botCapTex, seglen);
								if (planIsCCW && ! hasBackfaces)
					...
					else
					{
						...
						// NO SECTION PATH - must be flat poly
							...
							if (botCap)
							{
								...
								botMesh = AXPolygon.triangulate(resPolytree, topCapTex, seglen);
							if (planIsCCW && ! hasBackfaces)
									botMesh.triangles = botMesh.triangles.Reverse().ToArray();
								botMesh.RecalculateNormals();
			...
			else if (plan_p.paths != null)
			{
				...
				//Debug.Log ("Do Plan Paths: " + parametricObject.Name);
				...
				//Debug.Log("make plan paths: "+plan_p.paths.Count);
				...
				for (int i = 0; i < processedPaths2.Count; i++) {
					...
					float planArea =  AXGeometryTools.Utilities.pathArea(planPath);
					...
					if (secPaths != null && secPaths.Count > 0) {
						...
						for (int j = 0; j < secPaths.Count; j++) {
							...
							float sectionArea = AXGeometryTools.Utilities.pathArea(sectionPath);
							...
							else
							{
								...
								float appxVolumeHgt = Mathf.Abs(((float)(pbounds.top-pbounds.bottom)/((float)AXGeometryTools.Utilities.IntPointPrecision)));
...
//								topMesh.triangles = topMesh.triangles.Reverse ().ToArray ();
					...
					// @ --- CAP_B
					Matrix4x4 capBTransform = AXGeometryTools.Utilities.getEndTransformB (planPath, planSpline.isBeveledB);
					if (capB && endCapMeshDefault != null) {
		...
		public  GameObject generateGameObjectsFromPlanAndSecPolytrees(AXParametricObject initiator_po,  AXParameter plan_p, PolyNode planPolyTree, AXParameter sec_p, PolyNode secPolyTree, int gen=0)
		{
			...
			*/
			...
			foreach(PolyNode node in planPolyTree.Childs)
			{
					...
					{				
						...
						if (hasSides)
						{
							...
							{						
								...
								//AXGeometryTools.Utilities.calculateMeshTangents(ref holeMesh);
						...
						if (topCap)
						{
							if (topx != 0)
							{
								...
								co.AddPaths (AXGeometryTools.Utilities.cleanPaths(paths) , AXClipperLib.JoinType.jtMiter, AXClipperLib.EndType.etClosedPolygon); //JoinType.jtSquare, AXClipperLib.EndType.etClosedPolygon);
								co.Execute (ref resPolytree, (double)(topx * AXGeometryTools.Utilities.IntPointPrecision));
								if (resPolytree != null && resPolytree.Childs != null && resPolytree.Childs.Count > 0)
									topMesh = AXPolygon.triangulatePolyNode(resPolytree.Childs[0], topCapTex, seglen);
							}
							else 
							{
								
								topMesh = AXPolygon.triangulatePolyNode(node, topCapTex, seglen);
							}
						...
						if (botCap)
						{
							...
							if (botx != 0)
							{
								...
								co.Execute (ref resPolytree, (double)(botx * AXGeometryTools.Utilities.IntPointPrecision));
								if (resPolytree != null && resPolytree.Childs != null && resPolytree.Childs.Count > 0)
									botMesh = AXPolygon.triangulatePolyNode(resPolytree.Childs[0], botCapTex, seglen);
							}
							else 
								botMesh = AXPolygon.triangulatePolyNode(node, botCapTex, seglen);
						...
						if (capA && endCapMeshDefault != null)
						{
							Matrix4x4 capATransform = AXGeometryTools.Utilities.getEndTransformA(node.Contour);
						...
						if (capB && endCapMeshDefault != null)
						{
							Matrix4x4 capBTransform = AXGeometryTools.Utilities.getEndTransformB(node.Contour);
				...
				else
				{
					// just top or bottom caps
						...
						if (botCap)
						{
							...
							botMesh = AXPolygon.triangulatePolyNode(node, topCapTex, seglen);
				...
				GameObject  obj = parametricObject.makeGameObjectsFromAXMeshes(ax_meshes, false);
		...
		public void setSectionHandleTransform(AXParameter plan_p)
		{
			...
			if (planPaths != null && planPaths.Count > 0 && planPaths [0] != null) {
				...
				{
					Vector3 p0 = AXGeometryTools.Utilities.IntPt2Vec3 (firstPlanPath [0]);
					Vector3 p1 = AXGeometryTools.Utilities.IntPt2Vec3 (firstPlanPath [1]);
