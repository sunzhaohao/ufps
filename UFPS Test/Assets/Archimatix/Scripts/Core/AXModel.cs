...
namespace AX
{
	...
	public class AXModel : MonoBehaviour {
		...
		{
			AXGeometryTools.Utilities.setRealtimePrecisionFactor(precisionLevel);

		}
		public void setBuildPrecision()
		{
			AXGeometryTools.Utilities.setBuildPrecisionFactor(precisionLevel);
		}
...
//				}
		...
		//Debug.Log("Enable()");
			...
			*/
			...
			//selectAllVisibleInGroup(model.currentWorkingGroupPO);
...
//								Debug.Log("destroy " + gameObjectsToDelete[i].name);
		...
		public void doAnyDataMigration(AXParametricObject po)
		{
			...
			if (po.Type == "Turtle")
				po.Type = "Shape";
			else if (po.Type == "PairIterator")
				po.Type = "PairRepeater";
			else if (po.Type == "StepIterator")
				po.Type = "StepRepeater"; 
			else if (po.Type == "GridIterator")
				po.Type = "GridRepeater"; 
			else if (po.Type == "PlanSec")
				po.Type = "PlanSweep"; 
			else if (po.Type == "ShapeRepeater2D")
				po.Type = "GridRepeater2D"; 
...
//					}
		...
		public void selectAll()
		{
			...
			for (int i=0; i<parametricObjects.Count; i++)
			{
				...
				if (parametricObjects[i].isOpen)
				{
					...
					if (parametricObjects[i].generator is FreeCurve && ! activeFreeCurves.Contains(parametricObjects[i]))
		...
		public void selectPO(AXParametricObject po) 
		{
			...
			if (po.generator is FreeCurve && ! activeFreeCurves.Contains(po))
						activeFreeCurves.Add(po);
		...
		public void selectConnected(AXParametricObject po)
		{
			...
			foreach(AXParameter p in pList)
			{
				...
				if (p.Dependents != null)
				{
					...
					{
						if (d.PType == AXParameter.ParameterType.Output || d.Type == AXParameter.DataType.Mesh || d.Type == AXParameter.DataType.Spline)
		...
		*/
		...
		public void generateAlteredPOs()
		{
			...
			//Debug.Log("caching here B "+visitedPOs.Count);
		...
		public void setAXMatRecursive(AXParametricObject po, int governor = 0, AXParametricObject downstream_po = null)
		{

			/*
			if (downstream_po != null)
				Debug.Log(governor + " : " + po.Name + " :: " + downstream_po.Name + " downstream_po.axMat.mat=" + downstream_po.axMat.mat);
			else
...
//				}
			...
			*/
			...
			for (int i = 0; i < parametricObjects.Count; i++) {
				...
				if (po.is3D() && isTrailhead(po)) 
				{
					...
					GameObject go = po.generateOutputNow (true, po);
		...
		public void assertThumnailSupport()
		{
			...
			if (thumbnailMaterialMesh == null)
			{
				...
				AXGeometryTools.Utilities.calculateMeshTangents(ref thumbnailMaterialMesh);
		...
		public void cacheThumbnails(string caller = "unknown", bool makeTexture = false)
		{
			//return;
			...
			foreach (AXParametricObject po in ( (visitedPOs != null && visitedPOs.Count > 0) ? visitedPOs : parametricObjects))
			{
				...
				Thumbnail.render(po, makeTexture);	
